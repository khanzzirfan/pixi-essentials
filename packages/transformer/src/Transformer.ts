...
import {
  createHorizontalSkew,
  createVerticalSkew,
} from "./utils/skewTransform";
...
const tempCorners: [Point, Point, Point, Point] = [
  new Point(),
  new Point(),
  new Point(),
  new Point(),
...
/**
 * Color theme configuration for the transformer
 */
export interface ITransformerColorTheme {
  /** Primary color for handles and wireframe */
  primary?: number;
 ...
 */
export interface IRotatorAnchorConfig {
  /** Whether the rotator anchor is enabled */
  enabled?: boolean;
  /** Start position as percentage from bounding box (0-1) */
  startPosition?: number;
  /** Length of the line segment as percentage of total distance (0-1) */
  segmentLength?: number;
  /** Thickness of the anchor line */
  thickness?: number;
  /** Color of the anchor line (overrides theme) */
  color?: number;
  /** Style of the anchor line */
  style?: "solid" | "dashed" | "dotted";
  /** Dash pattern for dashed style [dashLength, gapLength] */
  dashPattern?: [number, number];
  /** Dot pattern for dotted style [dotSize, gapLength] */
...
export interface ITransformerOptions {
  ...
  /** Color theme configuration for the entire transformer */
  colorTheme?: ITransformerColorTheme;
...
const Container_ = Container as unknown as {
  new (): Container &
    IFederatedDisplayObject &
...
export class Transformer extends Container_ {
  ...
  protected _colorTheme: ITransformerColorTheme;
  ...
  constructor(options: Partial<ITransformerOptions> = {}) {
    ...
    const scaleHandles = SCALE_HANDLES.reduce(
      (scaleHandles, handleKey: ScaleHandle) => {
    ...
    this.handles = Object.assign(
      ...
      rotatorHandles,
      skewHandles
    ) as { [H in Handle]?: TransformerHandle };
  ...
  get colorTheme(): ITransformerColorTheme {
    return this._colorTheme;
  }
  set colorTheme(value: ITransformerColorTheme) {
    this._colorTheme = {
      primary: 0x6366f1,
      secondary: 0x4f46e5,
      background: 0xffffff,
  ...
  get rotatorAnchor(): IRotatorAnchorConfig {
    return this._rotatorAnchorConfig;
  }
  set rotatorAnchor(value: IRotatorAnchorConfig) {
    this._rotatorAnchorConfig = {
      enabled: true,
      startPosition: 0.45,
      segmentLength: 0.005,
  ...
  /**
   * Redraws all handles with updated styles
   */
  ...
  /**
   * Draws a line for the rotator anchor, connecting the bounding box to the rotator handle
   * @param x1 Start X coordinate (bounding box center)
   * @param y1 Start Y coordinate (bounding box center)
   * @param x2 End X coordinate (rotator handle position)
   * @param y2 End Y coordinate (rotator handle position)
   * @param lineWidth Width of the line
   * @param color Color of the line
   */
  private drawRotatorAnchorDot(
    x1: number,
    y1: number,
    x2: number,
    y2: number,
    ...
    // Calculate start and end points for disconnected line segment
    const segmentStartX = x1 + unitX * startPosition;
    const segmentStartY = y1 + unitY * startPosition;
    const segmentEndX = segmentStartX + unitX * segmentLength;
    const segmentEndY = segmentStartY + unitY * segmentLength;
    ...
    switch (config.style) {
      ...
      default:
        this.wireframe
          .lineStyle(anchorThickness, anchorColor)
          .moveTo(segmentStartX, segmentStartY)
          .lineTo(segmentEndX, segmentEndY);
        break;
    }
  }

  /**
   * Draws a dashed line between two points using the configured dash pattern
   */
  private drawDashedLine(
    ...
    const config = this._rotatorAnchorConfig;
    const [dashLength, gapLength] = config.dashPattern || [8, 4];
  ...
  /**
   * Draws a dotted line between two points using the configured dot pattern
   */
  private drawDottedLine(
    ...
    const config = this._rotatorAnchorConfig;
    const [dotSize, gapLength] = config.dotPattern || [2, 4];

    const dx = x2 - x1;
    const dy = y2 - y1;
    const distance = Math.sqrt(dx * dx + dy * dy);
    const dotCount = Math.floor(distance / (dotSize + gapLength));

    // Normalize direction vector
    const unitX = dx / distance;
    const unitY = dy / distance;

    this.wireframe.lineStyle(0).beginFill(color);

    for (let i = 0; i < dotCount; i++) {
      const dotX = x1 + unitX * i * (dotSize + gapLength);
      const dotY = y1 + unitY * i * (dotSize + gapLength);

      this.wireframe.drawCircle(dotX, dotY, dotSize);
    }
  ...
  rotateGroup = (handle: RotateHandle, pointerPosition: Point): void => {
    ...
    const handlePosition = this.worldTransform.apply(
      this.handleAnchors[handle],
      tempPoint
    );

    this.projectionTransform.applyInverse(handlePosition, handlePosition);
    pointerPosition = this.projectionTransform.applyInverse(
      pointerPosition,
      tempPointer
    );
    ...
    const orgAngle = Math.atan2(
      handlePosition.y - rOrigin.y,
      handlePosition.x - rOrigin.x
    );
    // Final tilt
    const dstAngle = Math.atan2(
      pointerPosition.y - rOrigin.y,
      pointerPosition.x - rOrigin.x
    );
  ...
  skewGroup = (handle: SkewHandle, pointerPosition: Point): void => {
    ...
    if (handle === "skewHorizontal") {
      ...
      // Calculate new skew
      const newSkew =
        Math.atan2(dst.y - sOrigin.y, dst.x - sOrigin.x) - Math.PI / 2;

      this._skewY = newSkew;
      this._skewY = this.snapAngle(
        this._skewY,
        this.skewSnapTolerance,
        this.skewSnaps
      );
  ...
  protected drawHandles(groupBounds: OrientedBounds): void {
    ...
    if (this._skewEnabled) {
      ...
      this.worldTransform.applyInverse(
        handleAnchors.skewHorizontal,
        handleAnchors.skewHorizontal
      );
      this.worldTransform.applyInverse(
        handleAnchors.skewVertical,
        handleAnchors.skewVertical
      );
  ...
  protected onPointerDown(e: FederatedPointerEvent): void {
    ...
    this._pointerMoveTarget = (this.stage || this) as unknown as DisplayObject &
      IFederatedDisplayObject;
        ...
        switch (this._transformHandle) {
          ...
          default: {
            if (this.translateEnabled) {
              ...
              this.projectionTransform.applyInverse(
                worldDestination,
                worldDestination
              );

              worldDelta.set(
                worldDestination.x - worldOrigin.x,
                worldDestination.y - worldOrigin.y
              );
   ...
   */
  private swapHandles(
    handle0: TransformerHandle,
    handle1: TransformerHandle
  ): void {
   ...
   */
  static calculateOrientedBounds(
    displayObject: DisplayObject,
    ...
    const corners = Transformer.calculateTransformedCorners(
      displayObject,
    ...
    bounds.innerBounds.x = Math.min(
      corners[0].x,
      corners[1].x,
      corners[2].x,
      corners[3].x
    );
    bounds.innerBounds.y = Math.min(
      corners[0].y,
      corners[1].y,
      corners[2].y,
      corners[3].y
  ...
  static calculateGroupOrientedBounds(
    ...
    for (let i = 0; i < groupLength; i++) {
      ...
      Transformer.calculateTransformedCorners(
        displayObject,
        displayObject.worldTransform,
        frames,
        i * 4
